...
export const Route = createFileRoute("/patrimoniu")({
  loader: () => fetchBuildings(),
  head: () => ({
    meta: [
      { title: "Patrimoniu — Ploieștiul Istoric Digital" },
      {
        name: "description",
        content:
          "Scanează un cod QR de pe o clădire sau răsfoiește arhiva pentru a descoperi istoria clădirilor istorice din Ploiești.",
      },
      { property: "og:title", content: "Patrimoniu — Ploieștiul Istoric Digital" },
      {
...
function Patrimoniu() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="mx-auto max-w-6xl px-4 pt-10 sm:pt-14 pb-24 flex-1 w-full">
        <div className="flex items-baseline justify-between border-b border-border/70 pb-3 mb-8">
          <h1 className="font-display text-2xl sm:text-3xl font-semibold">{t("home.archive")}</h1>
          {buildings && (
              ...
              {paginatedBuildings.map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5">
                      <h2 className="font-display text-xl sm:text-2xl font-semibold group-hover:text-primary transition-colors leading-tight">
                        {name}
                      </h2>
                      {address && (
